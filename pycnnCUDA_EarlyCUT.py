...
def plot_with_labels(lowDWeights, labels):
    ...
    plt.title('Visualize Dense layer(1024 dim)')
    plt.savefig('foo.png')
    plt.pause(0.01)
...
if __name__ == "__main__":
    ...
    for pshape in arg_list:
        ...
        optimizer = torch.optim.Adam(cnn_auto.parameters(), lr=LR)
        ...
        for epoch in range(EPOCH):
            ...
            for step, (b_x, b_y) in enumerate(batch(X_train, y_train, BATCH_SIZE)):
                b_x = b_x.cuda()
                b_y = b_y.cuda()
                cnn_output = cnn_auto(b_x)[0]
                auto_input = cnn_auto(b_x)[1]
                auto_output = cnn_auto(b_x)[2]
                loss = nn.CrossEntropyLoss()(cnn_output, b_y) + nn.MSELoss()(auto_input, auto_output)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
            ...
            cnn_auto.eval()
            with torch.no_grad():
                val_output, _, _ = cnn_auto(X_test)
                val_loss = nn.CrossEntropyLoss()(val_output, torch.from_numpy(y_test).cuda())

            print('Epoch: ', epoch, '\t| train loss: %.4f' % loss.data.item(), '\t| val loss: %.4f' % val_loss.item(), '\t time %.4f' % (time.time() - epoch_start))
